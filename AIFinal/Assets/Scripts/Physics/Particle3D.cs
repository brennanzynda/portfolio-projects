...
public class Particle3D : MonoBehaviour
{
    ...
    private float slowRadiusSq, stopRadiusSq;
    private float tempY;
   ...
   /*void UpdateAngularAcceleration()
    ...
    private void Move()
    {
        ...
        float diffSq = diff.sqrMagnitude;

        if (diffSq > slowRadiusSq)
        {
            // Fast As Fuk Boi
            diff.Normalize();
            diff *= maxAcc;
        ...
        {
            diff.Normalize();
            diff *= maxSpeed * (diffSq / slowRadiusSq);
            acceleration = Vector3.zero;
            velocity = diff;
        }
    }
    ...
    /*void CheckInput()
    {
        ...
        {
            //rotate CW by generating torque right from top thruster
            Vector3 newForce = ForceGenerator.GenerateForceInAxisDirection(rotationForce, topThruster.right);
        ...
        {
            //rotate CCW by generating torque left from top thruster
            Vector3 newForce = ForceGenerator.GenerateForceInAxisDirection(rotationForce, -topThruster.right);
            Vector3 pointOfForce = topThruster.position;
